...
namespace Front_TalleresSeta.Controllers
{
    public class TipoUsuariosController : Controller
    {
        ...
        public static string tabla = "TipoUsuarios";
        ...
        public TipoUsuariosController(IHttpClientFactory httpClientFactory, IFuncionRepositorio funcionRepo)
        {
            ...
            else
            {
                try
                {
                    ...
                    if (logueado.TipoUser == "SuperAdmin")
                        return View(await _httpClient.GetFromJsonAsync<List<TipoUsuario>>($"TipoUsuarios/todos?search={search}"));
                    else if (logueado.TipoUser == "Admin" || (logueado.TipoUser == "Empleado" && logueado.TipoRol == "Jefe"))
                        return View(await _httpClient.GetFromJsonAsync<List<TipoUsuario>>($"TipoUsuarios/todos?search={search}&tallerId={logueado.IdTaller}"));
                    else
                        return RedirectToAction("AccesoDenegado", "Interfaz", new { mensaje = "Acceso denegado al listado de productos" });
                }
                catch (Exception ex)
                {
                    mensaje = $"No se puede mostrar el o los tipos de usuario.";
            ...
            else
            {
                try
                {
                    if (!string.IsNullOrEmpty(id))
                    {
                        ...
                        var detalle = await _httpClient.GetFromJsonAsync<TipoUsuario>($"TipoUsuarios/obtener/{id}");
                    ...
                    {
                        return RedirectToAction("AccesoDenegado", "Interfaz", new { mensaje = $"Acceso denegado al detalle del tipo de usuario, el id: {id} no es válido o no existe en base de datos." });
                    }
                }
                catch (Exception ex)
                {
                    mensaje = $"No se puede mostrar el detalle del tipo de usuario.";
                    mensajeError = $"Error ocurrido: {ex.Message}";
                    return RedirectToAction("AccesoDenegado", "Interfaz", new { mensaje, mensajeError });
                }
            }
        ...
        public async Task<IActionResult> Create()
        {
            ...
            else
            {
                try
                {
                    ...
                    ViewData["mensaje"] = "Registra un tipo de usuario.";
                    return View();
                }
                catch (Exception ex)
                {
                    mensaje = $"No se puede crear un tipo de usuario.";
                    mensajeError = $"Error ocurrido: {ex.Message}";
                    return RedirectToAction("AccesoDenegado", "Interfaz", new { mensaje, mensajeError });
                }
            }
        ...
        public async Task<IActionResult> Create([Bind("Habilitado,FechaRegistro,TipoU,TallerId")] TipoUsuario model)
        {
            var logueado = _funcionRepo.ObtenerDatosLogueadoAsync();
            if (!logueado.IsAuth)
            {
            ...
            else
            {
                try
                {
                    ...
                    if (ModelState.IsValid)
                    {
                        ...
                        bool existe = await _httpClient.GetFromJsonAsync<bool>($"TipoUsuarios/existe/{model.TipoU}");
                        if (existe == false)
                        {
                            try
                            {
                                var responseCreate = await _httpClient.PostAsJsonAsync("TipoUsuarios/crear", model);
                                if (responseCreate.IsSuccessStatusCode)
                                {
                                    ...
                                    if (!responseObject.TryGetProperty("id", out JsonElement idElement))
                                    {
                                        ...
                                        mensaje = "Error: No se recibió el ID del tipo de usuario.";
                                    }
                                    ...
                                    else
                                    {
                                        ...
                                        mensaje = $"El tipo de usuario no se logró crear.";
                                    }
                                }
                                else
                                {
                                    mensaje = $"Error al crear el tipo de usuario: {responseCreate.StatusCode}";
                                }
                            }
                            catch (Exception)
                            {
                                mensaje = $"No se puede crear el tipo del usuario.";
                            }
                        }
                        else
                        {
                            mensaje = $"El tipo del usuario ya existe.";
                        }
                    }
                    else
                    {
                ...
                {
                    mensaje = $"No se puede crear el tipo de usuario.";
                    mensajeError = $"Error ocurrido: {ex.Message}";
                    return RedirectToAction("AccesoDenegado", "Interfaz", new { mensaje, mensajeError });
                }
                ViewData["TallerId"] = talleres;
        ...
        public async Task<IActionResult> Edit(string id)
        {
            ...
            else
            {
                try
                {
                    if (string.IsNullOrEmpty(id))
                    {
                        return RedirectToAction("AccesoDenegado", "Interfaz", new { mensaje = "No se puede modificar el tipo de usuario: id no válido." });
                    }

                    var model = await _httpClient.GetFromJsonAsync<TipoUsuario>($"TipoUsuarios/obtener/{id}");
                    if (model == null)
                    {
                        return RedirectToAction("AccesoDenegado", "Interfaz", new { mensaje = "No se encontró el tipo de usuario a modificar." });
                    }
                ...
                {
                    mensaje = $"No se puede modificar el tipo de usuario.";
                    mensajeError = $"Error ocurrido: {ex.Message}";
                    return RedirectToAction("AccesoDenegado", "Interfaz", new { mensaje, mensajeError });
                }
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(string id, [Bind("TipoUsuarioId, Habilitado,FechaRegistro,TipoU,TallerId")] TipoUsuario model)
        {
            accion = "ACTUALIZADO";
            ...
            else
            {
                if (ModelState.IsValid)
                {
                    try
                    {
                        var responseEdit = await _httpClient.PutAsJsonAsync($"TipoUsuarios/modificar", model);
                        if (!responseEdit.IsSuccessStatusCode)
                        {
                            mensaje = $"Error al actualizar el tipo de usuario: {responseEdit.StatusCode}";
                            return View(model);
                        }
                        ...
                        if (!responseObjectEdit.TryGetProperty("id", out JsonElement idElement))
                        {
                            ...
                            mensaje = "Error: No se recibió el ID del tipo de usuario al actualizar.";
                            return View(model);
                        }
        ...
        public async Task<IActionResult> DeleteConfirmed(string id)
        {
            ...
            else
            {
                try
                {
                    bool eliminado = await _httpClient.DeleteFromJsonAsync<bool>($"TipoUsuarios/eliminar/{id}");
                    if (eliminado)
                    {
                        accion = "ELIMINADO";
                        mensaje = "El tipo de usuario fue eliminado.";
                        ViewData["mensaje"] = mensaje;
                        return RedirectToAction("Index", new { mensaje, accion });
                    }
                    else
                    {
                        mensaje = "No se logró eliminar el tipo de usuario. Cuenta con registros asociados";
                        ViewData["mensaje"] = mensaje;
