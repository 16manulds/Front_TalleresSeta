...
namespace Front_TalleresSeta.Controllers
{
    public class InventarioSalidaProductosController : Controller
    {
        ...
        [HttpPut]
        public async Task<IActionResult> AgregarProductoVenta([FromQuery] long filtroId, [FromBody] DtoAgregarProducto model)
        {
            if (model == null) return BadRequest("El payload no puede ser nulo.");

            try
            {
                long idGenerado = await _inventarioSalidaProducto.AgregarProductoVentaAsync(filtroId, model);

                if (idGenerado <= 0) return BadRequest("No se pudo registrar el producto.");

                return Ok(new { id = idGenerado, productoId = idGenerado });
            }
            catch (Exception ex)
            {
        ...
        // DELETE: /InventarioSalidaProductos/EliminarPorId/5?filtroId=123
        [HttpDelete("InventarioSalidaProductos/EliminarPorId/{id}")]
        public async Task<IActionResult> EliminarPorId(long id, [FromQuery] long filtroId)
        {
            try
            {
                bool eliminado = await _inventarioSalidaProducto.EliminarPorIdAsync(id, filtroId);
